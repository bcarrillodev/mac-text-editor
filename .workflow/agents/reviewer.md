...
## Output
- review_report.json (must conform to `contracts/review_report.schema.json` and include `schema_version: "1.0.0"`)
- If the code is robust: Output JSON with `"decision": "MERGE_APPROVED"`
- If the code is flawed: Output JSON with `"decision": "REJECTED"` and a detailed, bulleted critique explaining exactly why it failed and what the Worker must fix in its next loop

## If FAIL:
Return critique → Worker loop
